...
```bash
curl -N -X POST https://api.cleveract.ai/v1/messages/stream \
  -H "Content-Type: application/json" \
...
Session variables are also injected into any [skills](/guides/skills) enabled on the bot. Sandbox scripts receive them as `VAR_` prefixed environment variables (e.g. `VAR_USER_ID`), and skill credentials can reference them with `{{variables.user_id}}` templates. This means your per-user data isolation variables can also drive skill behavior.
...
BASE = "https://api.cleveract.ai/v1"
