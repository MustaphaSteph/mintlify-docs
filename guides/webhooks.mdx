...
```bash
curl -X POST https://api.cleveract.ai/v1/chatbots/bot_abc/webhooks \
  -H "X-API-Key: ak_live_YOUR_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "url": "https://api.yourapp.com/webhooks/cleveract",
...
| `training.duplicate` | Upload skipped — content already exists | `source_type`, `duplicate_data_source_ids` or `duplicate_files`, `items_skipped` |
| `training.data_source_updated` | Data source metadata updated | `data_source_id`, `filename`, `metadata` |
...
<Tip>
  The `custom_metadata` field mirrors whatever you passed in the training request. Use it to identify which user or category the trained data belongs to.
</Tip>
...
The `training.duplicate` event fires when uploaded content is skipped because it already exists in the chatbot's knowledge base. This prevents your integration from waiting for a `training.completed` that never comes.

```json
{
  "event": "training.duplicate",
  "chatbot_id": "bot_abc123",
  "timestamp": "2025-01-15T10:30:00.000000Z",
  "data": {
    "source_type": "file",
    "duplicate_files": ["faq.csv (duplicate of ds_abc123)"],
    "items_skipped": 1
  }
}
```

For text and URL sources, the payload includes `duplicate_data_source_ids` instead of `duplicate_files`.
...
// Express example:
// app.post('/webhooks/cleveract', express.raw({type: 'application/json'}), (req, res) => {
...
When an event occurs, CleverAct AI delivers the webhook to your URL:
...
CleverAct AI tracks **consecutive failures** across all deliveries for each webhook:
...
```bash
curl -X PATCH https://api.cleveract.ai/v1/webhooks/wh_abc123 \
  -H "X-API-Key: ak_live_YOUR_KEY" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
