...
|-------|---------------|-------------|
| `conversation.started` | First message in a new session | `conversation_id`, `session_id` |
| `conversation.ended` | Conversation marked as resolved | `conversation_id`, `resolved_by`, `resolution_type` |
| `message.received` | AI sends a response | `conversation_id`, `session_id`, `tokens_in`, `tokens_out` |
| `handoff.triggered` | AI requests human agent | `conversation_id`, `session_id` |
| `handoff.resolved` | Agent transfers back to AI | `conversation_id`, `agent_id`, `reason` |
...
| `training.completed` | Training finishes successfully | `files_succeeded`, `total_chunks_stored`, `results` |
